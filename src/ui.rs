...
fn render_issue_list(f: &mut Frame, app: &mut App) {
    ...
    if app.issues.is_empty() {
        ...
        let empty = Paragraph::new(vec![Line::from(""), Line::from(empty_msg), Line::from("")])
        ...
        let items: Vec<ListItem> = app
            ...
            .map(|i| {
                ...
                let state =
                    Span::styled(format!("[{:?}]", i.state), Style::default().fg(state_color));
...
fn render_repo_selector(f: &mut Frame, app: &mut App) {
    ...
    if let Some(repo) = app.selected_repository_item() {
        ...
        let detail =
            Paragraph::new(detail_text).block(Block::default().borders(Borders::ALL).title("詳細"));
